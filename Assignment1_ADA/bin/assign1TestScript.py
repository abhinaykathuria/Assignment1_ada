#
# Script to perform automated testing for assignment 1 of AA, 2016.
#
...
def main():
    ...
    if bCompiled:
        ...
        for (j, sInLoopFile) in enumerate(lsInFile):
            ...
            sSearchOutputFile = os.path.join(
                sTestName + "-" + sImpl + ".search.out")
            ...
            with open(sOutputFile, "w") as fOut:
                ...
                sCommand = os.path.join(
                    "java " + sExec + " " + sImpl + " " + sSearchOutputFile)
...
#                 lCommand = os.path.join(sCodeDir, sExec + " " + sExpectedFile + ".test")
                ...
                with open(sInFile, "r") as fIn:
                    proc = sp.Popen(sCommand, shell=True,
                                    stdin=fIn, stdout=fOut, stderr=sp.PIPE)
                    ...
                    if sStderr == None:
                        ...
                        bSearchPassed = evaluateSearch(
                            sSearchExpectedFile, sSearchOutputFile)
                        if bPassed and bSearchPassed:
                            ...
                            if bVerbose:
                                if not bPassed:
                                    ...
                                    proc = sp.Popen(
                                        "diff -y " + sOutputFile + " " + sExpectedFile, shell=True)
                                    proc.communicate()
                                    print >> sys.stderr, ""
                                if not bSearchPassed:
                                    print >> sys.stderr, "Difference between search output and expected:"
                                    proc = sp.Popen(
                                        "diff -y " + sSearchOutputFile + " " + sSearchExpectedFile, shell=True)
                                    proc.communicate()
                                    print >> sys.stderr, ""
...
def usage(sProg):
    print >> sys.stderr, sProg + \
        "[-v] <code directory> <name of implementation to test> <list of test input files>"
    sys.exit(1)
